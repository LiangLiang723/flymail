...
`0.1.1` 已使用现有 `/Docker/flymail/data` 部署，根页面、静态资源和 API 404 边界均通过生产检查，数据目录没有重建、移动或清空。随后在真实局域网 HTTP 地址 `http://192.168.3.53:36080` 执行登录检查时发现：登录接口返回 200，但浏览器不会保存无条件带 `Secure` 的会话 Cookie，导致后续 `/api/v2/auth/me` 返回 401。该问题由 `0.1.2` 修复；本文件保留为根页面修复的历史证据。
