...
正式 Worker 入口当前默认创建空 `WorkerDispatcher`。在空数据库上它可以写入心跳并使健康检查通过，但没有装配 17 类生产任务处理器；真实任务出现后无法完成 IMAP、SMTP、OAuth、同步、正文、附件和邮件操作。

因此：

1. 当前结果不能支持生产 V2 切换。
2. 必须先完成正式 Worker 处理器和真实协议网关装配，或让容器在未装配时明确启动失败。
3. 必须提供隔离服务商账号、OAuth 客户端、测试代理、通知端点和真实浏览器/设备后重新执行本手册。
4. 在以上阻断解除前，生产容器和 `/Docker/flymail/data` 必须保持不变。
