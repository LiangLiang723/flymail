...
该接口对应同步管理页邮箱卡片里的“刷新同步”，会启动后台任务并连接 IMAP。任务按文件夹执行最近补新、正文补全、已读校正三步：

- 最近补新：每页拉取 50 封远端摘要，遇到本地已缓存 UID 后停止当前文件夹的最近补新阶段。
- 正文补全：只处理本地正文为空且 `body_checked = 0` 的缓存邮件。官方详情拉取成功后会写入正文/附件缓存，并将 `body_checked` 标记为已检查；官方正文确实为空的邮件不会在后续同步中反复拉取。
- 已读校正：用官方未读 UID 集合同步本地 `is_read`。

同步管理页右上角“刷新进度”不调用该接口，它只重新读取本地任务和文件夹进度。
