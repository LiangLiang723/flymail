...
- 使用当前仓库源码构建 `benxianyu/flymail:0.0.17` 单容器镜像
...
```bash
docker build -t benxianyu/flymail:0.0.17 .
```
...
```bash
docker push benxianyu/flymail:0.0.17
```
...
“继续”和“重试”都会尽量从上次断点恢复，不会默认从头重跑；如果执行“重置同步”，才会先删除本地记录后重新同步。运行中的历史同步每 30 秒写入一次任务心跳，即使正在下载单封大邮件也不会被 10 分钟无进度规则误判为失败；只有容器重启或后台任务真正中断且心跳停止超过 10 分钟时，列表才会将任务标记为失败并开放“重试”。同步管理页的“邮件摘要”只按本地摘要缓存计算，正文和附件补全阶段会单独显示，不会把摘要齐全误解为整个同步已经结束。

同步管理页右上角的“刷新进度”只重新读取本地任务状态和文件夹进度，不会连接 IMAP，也不会拉取官方邮件。“刷新同步”会启动后台同步任务；任务按文件夹执行三步：先每页 50 封拉取最近邮件，遇到本地已缓存 UID 就停止最近补新；再补全本地正文和附件；最后按官方未读 UID 集合同步本地已读/未读状态。页面会显示“正在检查最新邮件摘要”“正在补全正文和附件”或“正在同步已读状态”。IMAP 邮件详情已经包含附件二进制时会直接落盘，不会为了每个附件再次下载整封 MIME 邮件。`cached_messages.body_checked` 用来标记正文为空的邮件已经和官方详情比对过，避免正文真实为空的邮件每次都重复拉取详情。
