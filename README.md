...
V2 API Task 12 已增加管理员备份创建、列表、下载、完整性检查和隔离恢复演练。备份使用权限受限且 `0600` 的临时 MySQL 客户端配置，归档只包含数据库逻辑转储、内容寻址对象和不含密码的版本化清单；临时配置不会进入归档。检查过程拒绝绝对路径、路径穿越、符号链接、设备文件、重复或未声明成员，并逐项验证大小与 SHA-256。恢复演练只导入随机临时数据库，验证表数量和 schema 版本后立即删除临时数据库与文件；V2 API 不提供覆盖当前 `/data` 或生产数据库的直接恢复接口。该任务新增 schema 16 的 `backup_archives` 表。
