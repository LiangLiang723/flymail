...
Gate 5 Task 6 已在隔离 MySQL 8.0 数据库生成 50 用户、300 账号、2,000 万邮件摘要、500 万线程和 100 万正文搜索文档。4 核、约 31 GiB 可见内存、4 GiB InnoDB Buffer Pool 下，13 个 API/Worker 指标全部满足门槛；最慢的结构化搜索 P95 为 `27.460 ms`，混合 FULLTEXT P95 为 `14.901 ms`。线程列表、结构化搜索、任务领取、远端实例和配额 LRU 的 `EXPLAIN ANALYZE` 均使用承诺索引且无全表扫描。容量测试发现并修复了 ngram 处理长 ASCII 关键词时的候选缓存膨胀；原始两千万数据和日志未提交，生产 `/Docker/flymail/data` 未触碰。
