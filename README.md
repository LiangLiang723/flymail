...
基础层集成测试会从空数据库开始，验证 schema 迁移、API/Worker 独立连接池、事务回滚、凭证加密、内容寻址对象、Outbox 和任务租约。Gate 1 的 schema 基线为 `5`；邮件摘要摄取与公平调度分别加入增量索引和显式任务作用域后，当前 V2 开发 schema 为 `7`，健康检查按最新迁移版本进行验证。
