...
- V2 API、Worker、路由、线程、搜索、同步、通知和发送能力保持不变，正式前端恢复 V1 的完整视觉层：登录页、248/72px 全局侧栏、邮件工作台、表单、列表、卡片、弹窗、移动抽屉和全部一级页面统一使用 V1 紫色语义设计系统；非邮件路由不再被强制塞入邮件三栏
...
- 用户级 HTTP CONNECT 代理，覆盖 Gmail/Outlook OAuth、令牌刷新、IMAP 和 SMTP，并可由显式选择的通知渠道复用
...
- 用户级 HTTP CONNECT 代理只作用于该用户的 OAuth、IMAP、SMTP 和显式选择的通知渠道；内部 MySQL/API 不受影响。
- Worker 会按账号轮询间隔持续创建去重的对账任务；服务商支持 IDLE 与否不会影响服务器端持续同步，但真实 IDLE 长连接仍需独立服务商验收。
...
- 使用当前仓库源码构建 `benxianyu/flymail:0.1.4` 单容器镜像
...
```bash
docker build -t benxianyu/flymail:0.1.4 .
```
...
账号管理页可以配置新邮件后台轮询间隔，范围为 5 到 3600 秒。所有在线账号都会按该间隔创建去重的后台对账任务，持续补拉新摘要；生产 IDLE 长连接尚未装配，不能把轮询结果表述为实时推送。
...
V2 前端已成为正式 `src/main.ts` 入口，并继续使用 V2 Bootstrap、路由、状态模型、实时事件和 `/api/v2` 接口。`0.1.4` 起正式入口加载 V1 全量设计系统，由 V2 专用应用侧栏和兼容样式恢复 V1 外观；这不是切回旧 `App.vue` 或旧 API。源码验收可执行：
...
基础层集成测试会从空数据库开始，验证 schema 迁移、API/Worker 独立连接池、事务回滚、凭证加密、内容寻址对象、Outbox 和任务租约。Gate 1 的 schema 基线为 `5`；邮件摘要摄取、公平调度、精确 MIME 正文 part 元数据、可靠发送状态、通知临时图片引用、独立进程心跳、服务端认证会话、业务备份和混合全文索引分别加入增量结构后，当前 V2 开发 schema 为 `17`，健康检查按最新迁移版本进行验证。
...
V2 Worker 的注册表覆盖当前 17 类持久化任务：账号校验与清理、同步、正文/附件、离线操作、可靠发送、通知和配额清理。正式运行时通过显式 Handler 映射装配；映射缺项或数据库中出现未注册的可运行任务时，Worker 会在领取任务前拒绝启动。租约、心跳和安全关闭支持进程重启后继续 pending/retry 任务，一个账号的慢任务或失败不会阻塞其他账号。V2 原生运行时已接通 PKCE OAuth、用户代理、IMAP/SMTP TLS、摘要和 BODYSTRUCTURE 摄取、精确 MIME 抓取、读/星标/标签/移动/删除、可靠发送、Sent 查验与追加副本，并通过数据库驱动的周期轮询持续同步。真实邮箱凭证、服务商网络差异和生产 IDLE 仍需在真实服务商验收中确认，当前生产容器尚未替换。
...
V2 API Task 7 已增加结构化本地搜索、签名游标、搜索建议、最近历史和保存搜索 CRUD。搜索字段通过白名单编译为固定 SQL 片段，值全部参数化并始终包含用户范围；账号、文件夹、标签、日期、已读、星标、附件和大小筛选不依赖正文缓存。正文和索引元数据关键词使用 `body_search_documents` 混合 FULLTEXT：中文、日文、韩文关键词显式选择 ngram 索引，ASCII、数字和其他非 CJK 关键词显式选择标准解析器索引，两者均采用精确布尔短语；短词只在有界主题/地址元数据中回退，不对正文 HTML 做无界 LIKE。正文索引被驱逐后立即不再命中；搜索不会排队远端同步。建议只聚合当前用户的联系人、发件身份、原生标签和最近搜索，历史最多保留 50 条，保存搜索只落库经过模型验证的结构化 JSON。schema 17 解决了大规模英文正文在 ngram 候选缓存中膨胀的问题。
...
Gate 5 Task 6 已在隔离 MySQL 8.0 数据库生成 50 用户、300 账号、2,000 万邮件摘要、500 万线程和 100 万正文搜索文档。4 核、约 31 GiB 可见内存、4 GiB InnoDB Buffer Pool 下，绑定提交 `dc63431` 的 13 个 API/Worker 指标全部满足门槛；最慢的结构化搜索 P95 为 `24.754 ms`，混合 FULLTEXT P95 为 `13.966 ms`。线程列表、结构化搜索、任务领取、远端实例和配额 LRU 的 `EXPLAIN ANALYZE` 均使用承诺索引且无全表扫描。容量测试发现并修复了 ngram 处理长 ASCII 关键词时的候选缓存膨胀；原始两千万数据和日志未提交，生产 `/Docker/flymail/data` 未触碰。

Gate 5 Task 7 已建立真实服务商、代理、通知、图床、浏览器和无障碍验收手册及脱敏结果矩阵。本次 DevSpace 没有隔离邮箱、OAuth 客户端、代理、通知端点、图床、桌面浏览器、移动设备或屏幕阅读器，因此所有真实外部行均明确标记为 `blocked`，没有用假 Provider 或静态测试冒充真实通过。原始 Provider、协议和代理合同为 `79/79`，后续正式 Worker/OAuth/网络/同步/轮询组合回归累计 `100/100` 通过，PWA 与无障碍静态合同为 `10/10`。空 Dispatcher 阻断已经解除；真实账号、真实服务商网络和生产 IDLE 仍是外部验收阻断项。
...
Gate 5 Task 9 的历史发布证据保持不变。`0.1.3` 修复前端线程列表读取错误响应字段导致的登录后崩溃，并将所有用户输入的登录密码、管理员密码、邮箱密码或授权码、代理密码和业务备份密码统一为只要求非空、不限制长度且不自动去除首尾空格；`FLYMAIL_SESSION_SECRET` 等内部密钥仍保留安全最小长度。完整后端回归 `659/659`、兼容前端 `96/96`、V2 前端 `65/65`、生产构建、镜像级根页面、单字符管理员/普通用户登录、单字符备份密码、空线程列表、容器烟测和秘密扫描均通过。本地镜像为 `benxianyu/flymail:0.1.3`，镜像 ID 为 `sha256:62c0018193765510fb118309701574ff91f2eb84d8f76ddf50117837d6dc99c1`；Docker Hub 未上传，生产容器已使用该镜像运行。

Gate 5 Task 10 已于 2026-08-03 完成 V2 生产切换和 `0.1.3` 补丁部署。旧 `0.0.25` MySQL 安全关闭后，原数据被原子保留到 `/Docker/flymail/data-pre-v2-20260803T032435Z`，新的 `/Docker/flymail/data` 运行 schema 17。`flymail` 使用 `benxianyu/flymail:0.1.3` 完成根页面、静态资源、现有管理员登录、Bootstrap、线程 `items` 响应、重启持久化、Worker 心跳、MySQL 回环监听和日志秘密检查。部署过程曾因只读统计 SQL 使用错误表名触发一次自动回滚，确认旧 `0.1.2` 恢复健康后修正为 `mail_accounts` 并重新完成切换；生产数据未删除或迁移。新库尚未添加真实邮箱账号，外部服务商、发送收取和通知仍需使用真实账号后验收。

Gate 5 Task 11 已于 2026-08-03 在保留 V2 功能内核的前提下恢复 V1 全量 UI，并发布 `0.1.4`。V2 前端 `70/70`、兼容前端 `96/96`、隔离 MySQL 完整后端 `659/659`、TypeScript、生产构建、Playwright 桌面/移动视口烟测、Shell、Compose、镜像、复杂数据库密码容器持久化、秘密扫描和正式容器重启均通过。本地镜像为 `benxianyu/flymail:0.1.4`，镜像 ID 为 `sha256:5fdefe8f608de6a691cbaf9a94b00a3dfc8d5a0a0007975eb7b7dc81eec80c1f`；Docker Hub 未上传。正式 `flymail` 容器运行 healthy，使用 `/Docker/flymail/data:/data`、MySQL 8.0.46、schema 17，用户/邮箱账号/schema 计数在替换和重启后保持 `1:0:17`。部署期间旧容器不属于当前 Compose 项目导致一次同名冲突；保护回滚命令随后暴露了空会话密钥传递缺陷，入口脚本在 MySQL 启动前拒绝运行。原数据未删除或迁移，最终使用保留的数据库凭据和新的随机会话签名密钥启动 `0.1.4`；已有浏览器会话需要重新登录，但用户和邮件数据不受影响。
...
- [FlyMail 0.1.4 V1 全量 UI 恢复验证](docs/benchmarks/flymail-0.1.4-v1-ui-restoration.md)
