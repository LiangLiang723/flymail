...
V2 API Task 11 已增加通知中心、签名游标分页、未读计数、已读/忽略/全部已读和安全通知偏好接口。所有通知查询和状态修改均按用户隔离，响应不返回渠道密钥、Webhook 地址、令牌、密码或密文；变更通过 `notification.updated` 或 `settings.updated` 实时事件通知客户端。通知偏好只保存应用内/外部启用状态、图片开关、经过格式验证的安静时段和事件类型选择；既有 Bark、Telegram 等外部渠道秘密仍由加密通知 Worker 管理。该任务新增 schema 15 的 `notification_preferences` 表。
