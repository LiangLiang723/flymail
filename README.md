...
- 使用当前仓库源码构建 `benxianyu/flymail:0.1.1` 单容器镜像
...
```bash
docker build -t benxianyu/flymail:0.1.1 .
```
...
V2 API Task 13 已完成 Gate 3 安全、契约和纵向集成验收。所有 V2 HTTP 接口统一使用 `/api/v2` 前缀；本地认证采用服务端会话、签名 Cookie 与同源 CSRF 校验，修改密码后旧 V2 会话不可重放。远端收取、正文/附件获取、邮件操作、可靠发送、通知投递、配额清理和备份均通过持久化异步任务执行，HTTP API 只返回本地状态或任务标识；搜索始终限制在当前用户的本地投影与索引，不连接远端邮箱。业务备份只覆盖明确列出的配置和业务关系，并仅提供随机临时数据库与对象目录中的恢复演练，不提供覆盖生产 `/data` 的直接恢复接口。OpenAPI 已冻结为版本 `0.1.1`，包含 90 个路径、114 个操作和 138 个 schema，规范化 SHA-256 为 `51700b765067e28fb6ceb6f3e80499653680baa1adb40fa7485129a03c4fa84a`；跨租户资源猜测、伪造游标/删除令牌、CSRF/Origin、SQL 式搜索值、超大或活动图片和旧会话重放均有回归覆盖。最终隔离 MySQL 完整后端回归为 `654/654` 通过且无跳过。仓库正式 API/Worker 入口已切换到 V2；生产容器在补丁部署前仍运行 `0.1.0`。
...
Gate 5 Task 9 的 `0.1.0` 发布证据保持不变。补丁版本 `0.1.1` 修复根路径未提供前端页面的问题，完整后端回归 `654/654`、兼容前端 `96/96`、V2 前端 `64/64`、生产构建、镜像级根页面/静态资源/SPA 路由、容器烟测和秘密扫描均通过。本地镜像为 `benxianyu/flymail:0.1.1`，镜像 ID 为 `sha256:01911375be6f5e1852f2384b96693287cd3ca41f073cbe0ebd468c89ed4fb475`；Docker Hub 未上传，生产容器在补丁部署前仍使用 `0.1.0`。

Gate 5 Task 10 已于 2026-08-03 完成生产切换。旧 `0.0.25` MySQL 安全关闭后，原数据被原子保留到 `/Docker/flymail/data-pre-v2-20260803T032435Z`，新的 `/Docker/flymail/data` 运行 schema 17。`flymail` 在补丁部署前使用 `benxianyu/flymail:0.1.0`，健康、管理员登录、Bootstrap、数据库读写、Worker 心跳、重启持久化、MySQL 安全关闭和秘密扫描均通过。新库尚未添加真实邮箱账号，外部服务商、发送收取、通知和真实浏览器验收仍需后续执行。
