...
聚合收件箱默认关闭，每个用户可在“设置 → 聚合收件箱”中独立开启或关闭。开启后侧栏会显示入口，用户可继续独立选择参与聚合的邮箱账号，并支持跨账号查看、筛选和全部已读；关闭仅隐藏并停用聚合入口，不会删除已保存的账号选择、邮件缓存或邮箱数据。联系人同样按用户隔离，可从邮件详情快速添加，并在写信时提供地址自动补全。
