...
支持邮件 / 会话视图、关键词与高级筛选、未读状态、附件状态，以及常见数字验证码识别与一键复制。
...
提供主题、聚合收件箱、签名、附件缓存、定时清理以及 Gmail 代理等用户级设置。
...
- **邮件收发**：支持收件、回复、转发、草稿、抄送、密送、附件、内嵌图片以及定时发送。
...
- **邮件会话**：优先使用标准 `Message-ID`、`References` 和 `In-Reply-To` 组织会话，兼容旧缓存的保守主题回退。
- **验证码识别**：在主题和已缓存正文中识别常见 4–8 位数字验证码，列表中可直接复制；复制未读邮件中的验证码后会同步标记为已读。
- **附件缓存**：普通附件按需下载，内嵌图片随正文缓存；缓存对象按 SHA-256 去重，并支持用户级容量限制与 LRU 清理。
...
```bash
docker build -t benxianyu/flymail:0.0.58 .
```
