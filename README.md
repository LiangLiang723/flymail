...
- 使用当前仓库源码构建 `benxianyu/flymail:0.0.29` 单容器镜像
...
```bash
docker build -t benxianyu/flymail:0.0.29 .
```
...
```bash
docker push benxianyu/flymail:0.0.29
```
...
邮件管理页右上角的刷新会先拉取当前文件夹的最新一页并校正已读/未读状态，然后在后台按 UID 从新到旧分批补齐缺失的邮件摘要。远端刷新最长等待 45 秒；超时、历史同步或缓存维护占用账号时会立即返回现有本地缓存并显示原因，不会让按钮无限旋转。摘要补齐不下载正文和附件，每完成一批都会更新本地计数并通过 WebSocket 刷新页面；同一文件夹重复点击不会堆积重复任务，交互刷新到来时后台补齐会在当前批次结束后让行。实时监听和周期增量同步只处理新邮件、删除及状态变化，不会自行启动耗时的历史缺口补齐。正文和内嵌图片由历史同步、打开邮件或预取流程补全，普通附件只在点击时按需下载。
