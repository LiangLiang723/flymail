...
- 使用当前仓库源码构建 `benxianyu/flymail:0.0.28` 单容器镜像
...
```bash
docker build -t benxianyu/flymail:0.0.28 .
```
...
```bash
docker push benxianyu/flymail:0.0.28
```
...
同步管理页面的“邮件摘要”使用各文件夹本地缓存数与远端总数汇总，口径为 `sum(cached_count) / sum(total_count)`；下方各文件夹标签使用同一套 `cached_count / total_count` 数据。正文补全阶段另行计算当前文件夹中“已有正文或已确认正文为空”的邮件数，并显示 `正文已处理 / 已缓存摘要总数`，该计数不会混入邮件摘要进度。任务失败时页面显示“失败时间”，只有正常完成的任务才显示“完成时间”。
