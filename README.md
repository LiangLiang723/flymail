...
V2 API Task 10 已增加当前用户设置、联系人 CRUD/搜索和管理员历史同步控制接口。用户只能读取和修改自己的主题、密度、撰写偏好、远程图片策略及缓存配额；设置提交后写入安全审计并发布 `settings.updated` 实时失效事件。联系人按规范化邮箱在用户范围内唯一，列表、更新和删除均包含用户条件。历史同步列表仅管理员可访问，展示任务状态、账号、失败摘要及持久化游标；暂停、恢复和重试只切换本地 Worker 任务与租约状态，保留原始 payload、邮件数据和 `sync_cursors` 检查点，并写入管理员审计和 `sync.updated` 事件。
