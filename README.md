...
- 统一语义化设计系统覆盖浅色和深色主题，按钮、输入框、卡片、弹窗、状态提示与页面层级复用同一套组件和视觉规范；危险操作、附件操作、编辑器弹层、禁用态和移动端图标按钮均经过双主题全页面审计
- 高密度邮件工作台统一邮件、管理、分栏和设置文档四类页面模板；管理页采用 1280px 有界内容列和圆角工具面板，联系人等分栏页采用完整圆角外框，文档页保持 960px 阅读宽度；所有桌面端一级页面固定从侧栏后 20px 起始，不再因内容列居中产生不同左侧空白，移动工作区保持边到边适配，加载与空数据状态复用同一套组件
- 深色模式可在设置中选择跟随浏览器、浅色或深色，默认跟随浏览器，并对低动态、低透明和高对比偏好提供适配
- 登录后的全局导航采用 248px 展开、72px 折叠的无顶栏侧栏；折叠态保留居中的 FlyMail Logo，鼠标悬停或键盘聚焦品牌区时切换为展开按钮，其余图标继续严格居中且不会重叠；主侧栏只保留邮件工作流入口，用户头像菜单集中个人资料、用户管理、第三方通知、设置、关于、修改密码和退出登录；移动端通过屏幕左侧入口打开包含主导航、账号和文件夹的统一抽屉
...
- 使用当前仓库源码构建 `benxianyu/flymail:0.0.22` 单容器镜像
...
```bash
docker build -t benxianyu/flymail:0.0.22 .
```
...
```bash
docker push benxianyu/flymail:0.0.22
```
