...
- 缓存邮件正文和内嵌图片；多张 CID 正文图片会分别缓存和显示，不会混入附件列表；普通附件首次点击时按需下载并缓存
...
- 登录后的全局导航采用 248px 展开、72px 折叠的无顶栏侧栏；主侧栏只保留邮件工作流入口，用户头像菜单集中个人资料、用户管理、第三方通知、设置、关于、修改密码和退出登录；移动端通过屏幕左侧入口打开包含主导航、账号和文件夹的统一抽屉
...
- 使用当前仓库源码构建 `benxianyu/flymail:0.0.19` 单容器镜像
...
```bash
docker build -t benxianyu/flymail:0.0.19 .
```
...
```bash
docker push benxianyu/flymail:0.0.19
```
