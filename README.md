...
- 高密度邮件工作台统一邮件、管理、分栏和设置文档四类页面模板；桌面端所有一级页面使用一致的 20px 内容起始线，移动工作区保持边到边适配；页头与工具栏稳定，页面或面板拥有明确且唯一的滚动区域
...
- 使用当前仓库源码构建 `benxianyu/flymail:0.0.20` 单容器镜像
...
```bash
docker push benxianyu/flymail:0.0.20
```
