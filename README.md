...
- 登录后的全局导航采用无顶栏的扁平侧栏：通知中心和用户菜单位于侧栏底部；桌面端折叠后保留图标栏并记住状态，移动端通过屏幕左侧入口打开包含主导航、账号和文件夹的统一抽屉
...
- 使用当前仓库源码构建 `benxianyu/flymail:0.0.11` 单容器镜像
...
```bash
docker build -t benxianyu/flymail:0.0.11 .
```
...
```bash
docker push benxianyu/flymail:0.0.11
```
...
“继续”和“重试”都会尽量从上次断点恢复，不会默认从头重跑；如果执行“重置同步”，才会先删除本地记录后重新同步。同步任务超过 10 分钟没有任何进度时，列表会自动将其标记为失败并开放“重试”，避免容器重启或后台任务中断后长期显示“同步中”。同步管理页的已同步邮件数只按本地摘要缓存计算，打开邮件补全正文和附件不会改变这个数量。
