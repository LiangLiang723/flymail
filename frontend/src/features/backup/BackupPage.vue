...
<template>
  <main v-if="canAccessAdminRoute(bootstrap.state.data?.user.role || '')" class="v2-backup-page">
    ...
    <label>独立备份密码<input v-model="password" type="password" minlength="12" autocomplete="new-password" /></label>
    <button type="button" :disabled="password.length < 12" @click="safe(createBackup)">创建加密备份</button>
    <p v-if="state.error" class="v2-error">{{ state.error }}</p>
    <section><h2>备份列表</h2><article v-for="item in state.items" :key="item.id"><strong>{{ item.archive_name }}</strong><span>{{ item.status }} · {{ item.size_bytes }} 字节</span><button type="button" :disabled="password.length < 12" @click="safe(() => inspect(item))">检查范围与计数</button><a :href="`/api/v2/admin/backups/${encodeURIComponent(item.id)}/download`">下载</a></article></section>
    <section v-if="state.inspection"><h2>检查结果（确认演练前）</h2><dl><template v-for="(value, key) in state.inspection" :key="key"><dt>{{ key }}</dt><dd>{{ value }}</dd></template></dl><label>再次输入密码<input v-model="password" type="password" minlength="12" /></label><button type="button" :disabled="password.length < 12" @click="safe(rehearse)">执行 restore-rehearsal 隔离恢复演练</button></section>
